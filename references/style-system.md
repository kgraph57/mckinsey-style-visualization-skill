...
Every mark must carry information (data-ink rule). The one sanctioned decorative motif is the kicker bar above the headline — a single signature, used once per slide. Do not echo it as accent bars on annotations, column headers, or cards; a repeated motif reads as template filler and competes with the emphasis ladder. Concretely, this rules out: a colored left-edge accent bar on a KPI card (`kpi_scorecard` — status reads through the trend text color instead); a vertical divider line between `summary_strip` columns (whitespace does that job); and a `GREY_BORDER` stroke paired with a `GREY_FILL` fill on any bar or box (`gap`, `before_after`, `distribution`, `gantt`, `process_flow` — see Chart Rules).
...
- A left-edge accent bar or colored border on a card, tile, or panel — this is the kicker-bar motif again, wearing a different hat.
