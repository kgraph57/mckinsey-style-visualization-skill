# Launch Kit

Use this file to announce releases, collect demand signals, and avoid rewriting launch copy from scratch.

## Launch Goal

Get the skill in front of agent users who create business, strategy, board, investment, or consulting-style materials.

Primary outcome: marketplace listings, GitHub stars, forks, saves, replies, and inbound requests for rendered slide examples or paid templates.

## Release Copy

### Short Post

```text
I released Strategy Consulting Visualization Skill v1.4.0.

It turns messy business notes into board-ready visualization specs:
- ARR waterfalls
- 2x2 market maps
- benchmark tables
- capacity gap visuals
- investment memo visuals
- executive summary strips

The repo includes visual examples, validation, and a draft -> review -> revise loop.

https://github.com/kgraph57/mckinsey-style-visualization-skill
```

### X / Threads Version

```text
New release: Strategy Consulting Visualization Skill v1.4.0

Raw business notes -> board-ready slide specs.

Includes:
- 13 README visuals
- 4 draft/review/revise examples
- local validation
- marketplace submission docs
- buyer brief

https://github.com/kgraph57/mckinsey-style-visualization-skill
```

### LinkedIn Version

```text
I published Strategy Consulting Visualization Skill v1.4.0.

The skill helps AI agents turn raw business metrics into executive-ready slide specifications. It focuses on decision framing, insight-led headlines, chart pattern selection, assumptions, and review loops.

Use cases include board updates, market-entry analysis, vendor selection, investment memos, capacity gaps, competitive benchmarks, and executive summary visuals.

This release adds launch materials, marketplace targets, a buyer brief, and a social card so the package is easier to list, share, and evaluate.

Repository:
https://github.com/kgraph57/mckinsey-style-visualization-skill
```

### Community Post

```text
I built an open-source SKILL.md package for executive strategy visualization.

The goal is simple: give an AI agent enough structure to turn messy business inputs into board-ready slide specs instead of generic chart prompts.

What is included:
- portable SKILL.md entrypoint
- visual pattern library
- style system
- prompt templates
- quality rubric
- README visual gallery
- draft -> review -> revise examples
- local validation scripts
- marketplace submission docs

Repo:
https://github.com/kgraph57/mckinsey-style-visualization-skill
```

## Social Asset

Use `assets/social/launch-card.svg` as the launch image.

Alt text: Strategy Consulting Visualization Skill converts raw business notes into executive-ready slide specifications, with visual examples, review loops, and marketplace-ready proof.

## 7-Day Launch Plan

| Day | Action | Success Signal |
| --- | --- | --- |
| 1 | Post short copy on X/Threads and LinkedIn | Replies, reposts, bookmarks |
| 1 | Submit to P0 marketplace targets | Submission received or listing live |
| 2 | Post community version in relevant agent/Claude/Codex communities | Comments asking how it works |
| 3 | Publish one mini case study: raw notes -> draft -> review -> revised spec | Saves and example requests |
| 4 | Submit to P1 marketplace targets | Additional listings |
| 5 | Ask 5 target users to try a board-update prompt | Usability feedback |
| 6 | Add any requested example scenario | New issue, fork, or PR |
| 7 | Summarize metrics and decide premium-pack scope | Evidence for next release |

## Metrics

Track these weekly:

| Metric | Why It Matters |
| --- | --- |
| GitHub stars | Lightweight discovery and trust signal |
| Forks | Developer interest |
| Issues/discussions | Real usage and confusion points |
| Marketplace listings | Distribution footprint |
| Marketplace installs/saves | Demand signal |
| Social bookmarks | Buyer/user intent |
| Replies requesting PPTX/PDF | Premium pack signal |
| Direct messages | Sales and acquisition signal |

## Launch Claims To Avoid

- Do not claim affiliation with any consulting firm.
- Do not claim the skill renders final PowerPoint decks by itself.
- Do not claim marketplace approval before a listing is live.
- Do not claim revenue or usage traction until tracked.

## Follow-Up Case Study Format

```text
Input:
[Paste 4-8 lines of raw business notes]

Draft:
[Show the first slide-spec output]

Review:
[Show 3-5 review findings]

Revision:
[Show the revised decision headline, chart choice, assumptions, and quality score]
```
